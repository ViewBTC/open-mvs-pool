...
func (u *PayoutsProcessor) XXprocess() {
	if u.halt {
		log.Println("XXprocess Payments suspended due to last critical error:", u.lastFail)
		return
	}
	mustPay := 0
	minersPaid := 0
...
func (u *PayoutsProcessor) process() {
	if u.halt {
		log.Println("process Payments suspended due to last critical error:", u.lastFail)
		return
	}
	mustPay := 0
	minersPaid := 0
